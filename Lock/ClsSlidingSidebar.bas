...
Public Sub EnableSwipeGesture(Enabled As Boolean, GestureAreaSize As Int, FinalMovement As Byte)
	...
	If pnlGesture.IsInitialized Then
		Select sbPosition
			Case 0 'LEFT
				pnlGesture.SetLayout(pnlContent.Left, 0, GestureAreaSize, getParentHeight)
			Case 1 'RIGHT
				pnlGesture.SetLayout(pnlContent.Left + pnlContent.Width, 0, GestureAreaSize, getParentHeight)
			Case 2 'TOP
				pnlGesture.SetLayout(0, pnlContent.Top, getParentWidth, GestureAreaSize)
			Case 3 'BOTTOM
				pnlGesture.SetLayout(0, pnlContent.Top + pnlContent.Height, getParentWidth, GestureAreaSize)
		End Select
	Else
		pnlGesture.Initialize("")
		Select sbPosition
			Case 0 'LEFT
				sbParent.AddView(pnlGesture, pnlContent.Left, 0, GestureAreaSize, getParentHeight)
			Case 1 'RIGHT
				sbParent.AddView(pnlGesture, pnlContent.Left + pnlContent.Width, 0, GestureAreaSize, getParentHeight)
			Case 2 'TOP
				sbParent.AddView(pnlGesture, 0, pnlContent.Top, getParentWidth, GestureAreaSize)
			Case 3 'BOTTOM
				sbParent.AddView(pnlGesture, 0, pnlContent.Top + pnlContent.Height, getParentWidth, GestureAreaSize)
		End Select
...
Private Sub GetViewAtXY(X As Float, Y As Float) As Object
	For Each v As View In pnlContent.GetAllViewsRecursive 
    	If X >= v.Left AND X <= v.Left + v.Width AND Y >= v.Top AND Y <= v.Top + v.Height Then
			Return v
		End If	
  	Next
	Return Null
...
Private Sub Gesture_onTouch(ViewTag As Object, Action As Int, X As Float, Y As Float, MotionEvent As Object) As Boolean
	If Action = 0 Then
		Dim Obj As View = GetViewAtXY(pnlGesture.Left + X, Y)
		If Obj.IsInitialized AND (Obj.Tag <> "1") Then 
			Return False
		End If
